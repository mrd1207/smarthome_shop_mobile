<template>
  <div class="login">
    ...
    <div class="users">
      <div class="userp">
        <!-- 账号密码登录 -->
        <van-form @submit="onSubmit" class="form" v-if="state == 0">
          <div>
            <van-field
              v-model="username"
              name="user_name"
        ...
        <!-- 验证码 -->
        <van-form @submit="onSubmitcode" class="form" v-else>
          <div class="codelogin">
            <van-field
              name="tel"
              v-model="tel"
        ...
        <div class="tranBtn">
          ...
          <div class="bycode" @click="tranBtn">{{ stateMsg }}登录</div>
        </div>
      </div>
    </div>
    <!-- 验证码登录 -->
...
import { loginPwd, sendPhoneCode, loginCode } from "@/api/user";
...
export default {
  ...
  methods: {
    ...
    async onSubmit(values) {
      ...
      let userPwdParam = `name=${values.user_name}&pwd=${values.user_pwd}`;
      console.log("userPwdParam: ", userPwdParam);
      let res = await loginPwd(userPwdParam);
    ...
    async onSubmitcode(values) {
      ...
      // if (values.sms == this.relsms) {
        let phoneCodeParam = `phone=${values.tel}&code=${values.sms}`;
        console.log('phoneCodeParam: ', phoneCodeParam);
        loginCode(phoneCodeParam).then((res) => {
          console.log("res: ", res);
